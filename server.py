...
from keycloak_integration import get_token, verify_token, get_user_db_attrs
...
def mssql_query_tool(query: str, params=None, db_name="default"):
    ...
    try:
        return run_query(query, _normalize_params(params), db_name)
...
def mssql_insert_tool(table: str, data, db_name="default"):
    ...
    if auth:
        return auth
    try:
        return insert_row(table, data, db_name)
    except Exception as e:
        return {"status": "error", "reason": str(e)}
...
def mssql_update_tool(table: str, data, condition, db_name="default"):
    ...
    if auth:
        return auth
    try:
        return update_row(table, data, condition, db_name)
    except Exception as e:
        return {"status": "error", "reason": str(e)}
...
def mssql_delete_tool(table: str, condition, db_name="default"):
    ...
    if auth:
        return auth
    try:
        return delete_row(table, condition, db_name)
    except Exception as e:
        return {"status": "error", "reason": str(e)}
...
def mssql_schema_tool(table_name: str, db_name="default"):
    ...
    if auth:
        return auth
    try:
        return get_table_schema(table_name, db_name)
    except Exception as e:
        return {"status": "error", "reason": str(e)}
    ...
    while True:
        ...
        try:
            ...
            dbs = list_all_databases(
                creds["db_user"], creds["db_password"],
                creds["db_server"], creds["db_port"], creds["db_driver"]
            )
...
if __name__ == "__main__":
    cli_login()
    logger.info(f"🚀 Starting MSSQL MCP Server")
